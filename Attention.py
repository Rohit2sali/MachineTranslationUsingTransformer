...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, padding_mask, lookahead_mask, n_heads, masked = bool):
        ...
        attention_scores = torch.matmul(q, k.transpose(-2, -1))/torch.sqrt(torch.tensor(d_k, dtype=torch.float32)) # (-2, -1) will swap the last two dimension of the matrix
        if(masked == True):
